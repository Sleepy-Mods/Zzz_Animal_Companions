...
namespace DireWolfMod
{
	...
	public static class CompanionPatches
	{
		...
		private const string RemoveSaddleReq = "dwRemoveSaddleReq";
		...
		public static void OnAddedToWorld_Post(EntityAlive __instance)
		{
			try
			{
				...
				// Server-only: bind owner and enforce one companion per owner
				if (!self.isEntityRemote)
				{
				...
				// Attach saddle visual on both server and clients if already saddled
				try
				{
		...
		{
			// Server-only behaviors
			try { if (__instance != null && !__instance.isEntityRemote) RunFollowAssist(__instance); } catch { }
			try { if (__instance != null && !__instance.isEntityRemote) ProcessInstallRequests(__instance); } catch { }

			// Visual sync on both server and clients
			try
			{
				var self = __instance;
				if (self == null || self.EntityClass == null) return;
				...
				if (saddled == 1)
				{
					...
					int syncFlag = 0; try { syncFlag = (int)self.Buffs.GetCustomVar("dwSaddleSync"); } catch { syncFlag = 0; }
					if (needApply == 1 || syncFlag == 1)
					{
						SaddleAssets.TryAttachSaddle(self);
						try { if (needApply == 1) self.Buffs.SetCustomVar(SaddleReapplyCvar, 0); } catch { }
						try { if (syncFlag == 1) self.Buffs.SetCustomVar("dwSaddleSync", 0); } catch { }
					}
				}
			}
			catch { }
		...
		private static void RunFollowAssist(EntityAlive self)
		{
			...
			if (dist > 40f)
			{
				...
				// Snap to ground to avoid midair collisions near vehicles/gyros
				try
				{
					var chunk = GameManager.Instance?.World?.GetChunkFromWorldPos((int)p.x, (int)p.y, (int)p.z);
					if (chunk != null)
					{
						int th = chunk.GetTerrainHeight(((int)p.x) & 0xF, ((int)p.z) & 0xF);
						float gy = Mathf.Max((float)th + 0.2f, 0.2f);
						p.y = gy;
					}
				}
				catch { }
		...
		private static void ProcessInstallRequests(EntityAlive self)
		{
			...
			bool reqBags = false;
			bool reqRemove = false;
			try { reqSaddle = (buffs.GetCustomVar(InstallSaddleReq) > 0f); } catch { }
			try { reqBags = (buffs.GetCustomVar(InstallBagsReq) > 0f); } catch { }
			try { reqRemove = (buffs.GetCustomVar(RemoveSaddleReq) > 0f); } catch { }
			if (!reqSaddle && !reqBags && !reqRemove) return;
			...
			if (reqSaddle)
			{
				...
				// Removed post-install tooltip per user request
			}
			if (reqBags)
			{
			...
			if (reqRemove)
			{
				buffs.SetCustomVar(SaddledVar, 0);
				buffs.SetCustomVar(StorageTierVar, 0);
				buffs.SetCustomVar(RemoveSaddleReq, 0);
				SaddleAssets.TryDetachSaddle(self);
				try { buffs.SetCustomVar("dwSaddleSync", 1); } catch { }
			}
		}

		// Animal loot drops: 30% chance to drop species item on death
		[HarmonyPatch(typeof(EntityAlive), "OnEntityDeath")]
		[HarmonyPostfix]
		public static void OnEntityDeath_Post(EntityAlive __instance)
		{
			TryDropAnimalLoot(__instance);
		}



		private static void TryDropAnimalLoot(EntityAlive dead)
		{
			try
			{
				if (dead == null || dead.EntityClass == null) return;
				// Ignore players and companions
				var ecn = dead.EntityClass.entityClassName ?? string.Empty;
				...
				else if (string.Equals(ecn, "animalSnake", StringComparison.OrdinalIgnoreCase)) itemName = "itemSnakeScale";
				else if (string.Equals(ecn, "animalDoe", StringComparison.OrdinalIgnoreCase)) itemName = "itemDoeTail";
				else if (string.Equals(ecn, "animalStag", StringComparison.OrdinalIgnoreCase)) itemName = "itemBuckAntler";
				else if (string.Equals(ecn, "animalRabbit", StringComparison.OrdinalIgnoreCase)) itemName = "itemRabbitFoot";
				if (string.IsNullOrEmpty(itemName)) return;

				var iv = ItemClass.GetItem(itemName, true);
				if (iv.IsEmpty()) return;
				var stack = new ItemStack(iv, 1);
				Vector3 dropPos = dead.position + new Vector3(0f, 0.2f, 0f);
