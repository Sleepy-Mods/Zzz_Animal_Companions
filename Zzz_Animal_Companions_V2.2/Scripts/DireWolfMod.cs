	...
	public static class ServerSpawnPatch
	{
		...
		public static void PlayerLive_Post(EntityPlayer __instance)
		{
			try
			{
				...
				// Spawn on server near player at safe ground height to avoid midair collisions
				Vector3 pos = player.position + new Vector3(1.25f, 0.1f, 0f);
				try
				{
					var chm = GameManager.Instance?.World?.GetChunkFromWorldPos((int)pos.x, (int)pos.y, (int)pos.z)?.GetTerrainHeight((int)pos.x & 0xF, (int)pos.z & 0xF) ?? (int)pos.y;
					float groundY = Mathf.Max((float)chm + 0.2f, 0.2f);
					if (pos.y < groundY || pos.y - groundY > 3.0f) pos.y = groundY;
				}
				catch { }
		...
		private static void HandleInstallRequests(EntityPlayer player)
		{
			try
			{
				...
				try { reqRemove = (player.Buffs.GetCustomVar("dwRemoveSaddleReq") > 0f); } catch { }
				...
				if (wolf != null)
				{
					...
					if (reqRemove) wolf.Buffs?.SetCustomVar("dwRemoveSaddleReq", 1);
				}
				...
				try { if (reqRemove) player.Buffs.SetCustomVar("dwRemoveSaddleReq", 0); } catch { }
			}
			catch { }
		}
	...
	public static class SaddleAssets
	{
		...
		public static void TryAttachSaddle(EntityAlive wolf)
		{
			try
			{
				...
				if (existing != null)
				{
					existing.transform.localPosition = new Vector3(0f, 0.66f, -0.1f);
					existing.transform.localEulerAngles = new Vector3(-20f, 0f, 0f);
					existing.transform.localScale = Vector3.one * 0.4f;
					return;
				}
				...
				go.transform.localPosition = new Vector3(0f, 0.66f, -0.1f);
				go.transform.localEulerAngles = new Vector3(-20f, 0f, 0f);
